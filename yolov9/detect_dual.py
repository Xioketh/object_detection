...
# Then:
ckpt_path = ROOT / 'deep_sort_pytorch' / 'deep_sort' / 'deep' / 'checkpoint' / 'ckpt.t7'
deepsort = DeepSort(str(ckpt_path))
...
def draw_boxes(frame, bbox_xyxy, draw_trails, identities = None, categories=None, offset=(0,0)):
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        # print('--------------------------------')
        # print(f'cat: {cat}, className length: {len(className)}, className: {className}')
        ...
        if draw_trails:
            for i in range(1, len(data_deque[id])):
                ...
                # draw trails
                cv2.line(frame, data_deque[id][i - 1], data_deque[id][i], (0, 0, 255), thickness)
    return frame
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                output = deepsort.update(xywhs, confss, oids, im0)
                if len(output)>0:
                    bbox_xyxy = output[:,:4]
                    identities = output[:,-2]
                    object_id = output[:,-1]
